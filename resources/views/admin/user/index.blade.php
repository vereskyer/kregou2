                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @foreach ($users as $user)
                                <tr class="text-gray-700 dark:text-gray-400">
                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                                <img class="object-cover w-full h-full rounded-full"
                                                    src="{{ $user->image ? asset("/storage/$user->image") : asset('uploads/default-store.jpg') }}"
                                                    alt="" loading="lazy" />
                                                {{-- <img class="object-cover w-full h-full rounded-full"
                                    ...
                                    <td class="px-4 py-3 text-sm">{{ $user->balance }}</td>
                                    <td class="px-4 py-3 text-sm">{{ $user->instagram }}</td>
                                    <td class="px-4 py-3 text-sm">{{ $user->website }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            <form action="{{ route('admin.users.edit', $user->id) }}" method="put">
                                                @csrf
                                                @method('post')
                                                <button
